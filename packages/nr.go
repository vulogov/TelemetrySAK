...
func Traces(nrikey string, url string, compress bool, _payload []byte) {
  var payload []byte
  var b bytes.Buffer
  if compress {
    log.Trace("Compression: ON")
...
func Logs(nrikey string, url string, compress bool, _payload []byte) {
  var payload []byte
  var b bytes.Buffer
  if compress {
    log.Trace("Compression: ON")
...
func init() {
  env.Packages["nr"] = map[string]reflect.Value{
    ...
    "Traces":                 reflect.ValueOf(Traces),
    "Logs":                   reflect.ValueOf(Logs),
  }
}
